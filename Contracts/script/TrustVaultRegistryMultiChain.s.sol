...
import {TrustVaultRegistry} from "../src/TrustVaultRegistry(MultiChain).sol";

contract DeploySepoliaMultiChain is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");

        vm.startBroadcast(deployerPrivateKey);

        console2.log("Deploying TrustVaultRegistry (MultiChain) to Sepolia...");
